...
<div class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">Edit Event</h1>
            <p class="mt-2 text-sm text-slate-500">Perbarui informasi event dan poster jika diperlukan.</p>
        </div>

        <a href="{{ route('partner.events.index') }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
    ...
    <form action="{{ route('partner.events.update', ['event' => $event->id]) }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="editEventForm" novalidate>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 p-5 sm:p-6">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-500">Edit detail event dan poster.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid gap-6 p-5 sm:p-6 md:grid-cols-2">
                ...
                <div class="space-y-2">
                    ...
                    >
                        ...
                        @foreach ($categories as $category)
                            ...
                            @endphp
                            <option value="{{ $value }}" {{ old('category', $event->category?->name ?? '') == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input
                        ...
                        name="event_datetime"
                        type="datetime-local"
                        value="{{ old('event_datetime', $event->date?->format('Y-m-d\TH:i')) }}"
                        required
                        class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 shadow-sm outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-100"
                    >
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            min="0"
                            step="1000"
                            value="{{ old('ticket_price', $event->price) }}"
                            placeholder="0"
                            required
                ...
                <div class="space-y-2">
                    ...
                    <input
                        ...
                        type="number"
                        min="1"
                        value="{{ old('ticket_quantity', $event->stock) }}"
                        placeholder="100"
                        required
                        class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 shadow-sm outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-100"
                    >
            ...
            <div class="flex flex-col-reverse gap-3 border-t border-slate-200 bg-slate-50/60 px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
                <a href="{{ route('partner.events.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                    Batal
                </a>
